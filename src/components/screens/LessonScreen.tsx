        ...
        return (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              >
                <span>{offlineDownload ? '✅' : '📱'}</span>
                <span>{offlineDownload ? 'Yükləndi' : 'Yüklə'}</span>
              </button>
    ...
    <div className="p-3 pb-32">
      ...
      <div className="relative mb-3 z-20">
        <button
          ...
          className={`w-full px-4 py-3 rounded-xl text-left flex items-center justify-between min-h-[44px] focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
            isDarkMode
              ? 'bg-gray-800 border border-gray-700 text-gray-100'
              : 'bg-white border border-gray-300 text-gray-900'
          }`}
        >
          <span className="font-medium flex-1 min-w-0 truncate">{displayTitle}</span>
          <span className={`transform transition-transform ${moduleDropdownOpen ? 'rotate-180' : ''}`}>
            ▼
          </span>
        </button>
        
        <div className={`mt-2 px-1 text-base font-bold leading-snug ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
        ...
        {lessonTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-bold whitespace-nowrap min-h-[36px] transition-all duration-200 ${
                isActive
                  ? is3DTab
            ...
            >
              ...
              <span>{tab.label}</span>
              {is3DTab && <div className="w-1.5 h-1.5 bg-yellow-400 rounded-full"></div>}
            </button>
          );
        })}
      </div>
