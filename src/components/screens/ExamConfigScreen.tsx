...
export function ExamConfigScreen() {
	const { t, navigate, isDarkMode, currentScreen, tickets } = useApp();
					...
					<Card>
						<div className="space-y-3">
							...
							<div className={`overflow-hidden transition-all duration-300 ${showPromo ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
								<div className={`mt-1 p-2 rounded-lg border flex items-center gap-2 ${
									promoStatus === 'err'
									...
									<>
										{promoStatus === 'ok' ? (
											<div className="text-xs mt-1 text-emerald-600">
												Promokod uğurla tətbiq edildi.Tətbiq edilən endirim: 2.00 AZN (misal olaraq)  Yekun qiyməti: 0.00 AZN
											</div>
										) : (
											<div className="text-xs mt-1 text-red-600">
												Promokodun istifadə müddəti bitib və ya bu hesabda istifadə edilə bilməz.
											</div>
										)}
										{promoStatus === 'ok' && (
											<div className={`text-[11px] mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
												Promokoddan simulyator balansı 0 olduqda istifadə etmək mümkündür.
											</div>
										)}
									</>
								)}
