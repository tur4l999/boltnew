import type { Language } from './types';

export const dictionaries = {
  az: {
    // Common
    hello: 'Salam',
    home: 'Ana',
    topics: 'Təlim mövzuları',
    exam: 'İmtahan',
    mistakes: 'Səhvlərim',
    store: 'Mağaza',
    more: 'Daha çox',
    
    // Actions
    quickTest: 'Sürətli test',
    videoLessons: '3D video dərs',
    onlineLesson: 'Onlayn dərslər',
    continue: 'Haradan davam edim?',
    startLesson: 'Dərsə başla',
    startTest: 'Testə başla',
    goToExam: 'İmtahana keç',
    next: 'Sonrakı',
    prev: 'Geri',
    finish: 'Bitir',
    examSimulator: 'İmtahan simulyatoru',
    tests: 'Biletlər',
    fines: 'Cərimələr',
    articles: 'Qaydalar',
    myResults: 'Nəticələrim',
    notes: 'Konspektlər',
    
    // Lesson tabs
    article: 'Qayda',
    video: 'Video dərs',
    materials: 'Dərs materialları',
    questions: 'Suallar',
    byTickets: 'Biletlər üzrə',
    byTopics: 'Mövzular üzrə',
    video3d: '3D Video',
    contactTeacher: 'Müəllimlə əlaqə',
    
    // Exam
    examSettings: 'İmtahan tənzimləmələri',
    topic: 'Mövzu',
    mixed: 'Qarışıq',
    questionsCount: 'Sual sayı',
    startExam: 'İmtahana başla',
    timeLeft: 'Qalan vaxt',
    confirmAnswer: 'Cavabı təsdiq et',
    explanation: 'İzah',
    
    // Results
    yourScore: 'Sənin balın',
    weakTopics: 'Zəif mövzular',
    learnFromMistakes: 'Səhvlərimi işlət',
    pass: 'Keçdin',
    fail: 'Keçmədin',
    allResults: 'Hamısı',
    ticketsExam: 'Biletlər üzrə',
    topicsExam: 'Mövzular üzrə',
    simulatorExam: 'Simulyator İmtahanı',
    finalExam: 'Yekun İmtahan',
    allDates: 'Bütün tarixlər',
    today: 'Bu gün',
    thisWeek: 'Bu həftə',
    thisMonth: 'Bu ay',
    date: 'Tarix',
    examType: 'İmtahan növü',
    totalScore: 'Ümumi keçmə',
    weeklyExams: 'Bu həftə',
    filters: 'Filterlər',
    noResults: 'Heç bir nəticə tapılmadı',
    passed: 'Keçdi',
    failed: 'Keçmədi',
    minutes: 'dəqiqə',
    
    // Store
    addToCart: 'Səbətə at',
    checkout: 'Ödəniş et',
    purchased: 'Ödəniş tamamlandı',
    
    // Other
    filterByTopic: 'Mövzu seç',
    modules: 'Modullar',
    progress: 'İrəliləyiş',
    download: 'Offline saxla',
    speed: 'Sürət',
    notifications: 'Bildirişlər',
    assistant: 'AI köməkçi',
    language: 'Dil',
    upcomingLessons: 'Davam edən Onlayn dərslər',
    classSchedule: 'Dərs cədvəli',
  },
  
  ru: {
    // Common
    hello: 'Привет',
    home: 'Главная',
    topics: 'Темы обучения',
    exam: 'Экзамен',
    mistakes: 'Мои ошибки',
    store: 'Магазин',
    more: 'Ещё',
    
    // Actions
    quickTest: 'Быстрый тест',
    videoLessons: '3D видео урок',
    onlineLesson: 'Онлайн урок',
    continue: 'Где продолжить?',
    startLesson: 'Начать урок',
    startTest: 'Начать тест',
    goToExam: 'К экзамену',
    next: 'Далее',
    prev: 'Назад',
    finish: 'Завершить',
    examSimulator: 'Симулятор экзамена',
    tests: 'Тесты',
    fines: 'Штрафы',
    articles: 'Статьи',
    myResults: 'Мои результаты',
    notes: 'Конспекты',
    
    // Lesson tabs
    article: 'Статья',
    video: 'Видео урок',
    materials: 'Материалы',
    questions: 'Вопросы',
    byTickets: 'По билетам',
    byTopics: 'По темам',
    video3d: '3D Видео',
    contactTeacher: 'Связь с преподавателем',
    
    // Exam
    examSettings: 'Настройки экзамена',
    topic: 'Тема',
    mixed: 'Смешанные',
    questionsCount: 'Кол-во вопросов',
    startExam: 'Начать экзамен',
    timeLeft: 'Осталось',
    confirmAnswer: 'Подтвердить ответ',
    explanation: 'Пояснение',
    
    // Results
    yourScore: 'Твой балл',
    weakTopics: 'Слабые темы',
    learnFromMistakes: 'Учить ошибки',
    pass: 'Сдал',
    fail: 'Не сдал',
    allResults: 'Все',
    ticketsExam: 'По билетам',
    topicsExam: 'По темам',
    simulatorExam: 'Симулятор экзамена',
    finalExam: 'Финальный экзамен',
    allDates: 'Все даты',
    today: 'Сегодня',
    thisWeek: 'Эта неделя',
    thisMonth: 'Этот месяц',
    date: 'Дата',
    examType: 'Тип экзамена',
    totalScore: 'Общий процент',
    weeklyExams: 'На этой неделе',
    filters: 'Фильтры',
    noResults: 'Результаты не найдены',
    passed: 'Сдал',
    failed: 'Не сдал',
    minutes: 'минут',
    
    // Store
    addToCart: 'В корзину',
    checkout: 'Оплатить',
    purchased: 'Оплачено',
    
    // Other
    filterByTopic: 'Выбор темы',
    modules: 'Модули',
    progress: 'Прогресс',
    download: 'Сохранить офлайн',
    speed: 'Скорость',
    notifications: 'Уведомления',
    assistant: 'AI помощник',
    language: 'Язык',
    upcomingLessons: 'Ближайшие занятия',
    classSchedule: 'Расписание занятий',
  },
};